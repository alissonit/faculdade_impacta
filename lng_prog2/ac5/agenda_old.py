...
class Email:
    ...
    def valida_email(email: str) -> bool:
        ...
        if len(email.split("@")) > 2:
            raise ValueError("Email deve conter somente uma @.")    
        
        clean_email = email.replace("@").split(".").join()
        clean_email = [resp for resp in clean_email if not resp.isalnum()]
        if len(clean_email) > 0:
            raise ValueError("Deve conter apenas letras, números e pontos, senão levanta um")
    ...
    def eh_aluno_impacta(self) -> bool:
        ...
        """
        if self._domain == "aluno.faculdadeimpacta.com.br":
            return True
        else:
            return False    
    ...
    def eh_impacta(self) -> bool:
        ...
        """
        if self._domain == "faculdadeimpacta.com.br":
            return True
        else:
            return False
