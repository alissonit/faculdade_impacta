...
#
# e-mails: alisson.castro@aluno.faculdadeimpacta.com.br
...
class Contato:
    ...
    def create_dump(self):
        ...
        exp_contato = {}
        exp_contato["nome"] = self.nome
        exp_contato["telefones"] = self.get_telefones()
        exp_contato["emails"] = self.get_emails()
...
class Agenda:
    ...
    def ligar(self, valor_busca, tipo='principal') -> None:
        ...
        """
        contatos_encontrados = self.busca_contatos(valor_busca)

        for contato in contatos_encontrados:
            telefones_contato = contato.lista_telefones()
    ...
    def apagar_contato(self, email_busca) -> str:
        ...
        for contato in self.contatos:
            telefones_contato = contato.lista_emails()
            for email in telefones_contato:
    ...
    def carregar_contatos(self, nome_arquivo: str) -> None:
        ...
        with open(nome_arquivo, 'r') as arquivo:
            arquivo = json.loads(arquivo.read())
            for contato in arquivo:
                nome = contato['nome']
                a = Contato(nome, contato['telefones']['principal'], contato['emails']['principal'])
                for tipo, telefone in contato['telefones'].items():
                    if tipo not in ['principal']:
                        if tipo in contato['emails']:
                            a.adiciona_email(contato['emails'][tipo], tipo)
                        a.adiciona_telefone(telefone, tipo)
                if a.buscar(nome):
                    print(f'contato: {a.nome} adicionado!')

if __name__ == "__main__":
    a = Agenda('Rafael', '11999887766', 'rafael@email.com')

    a.carregar_contatos('exemplo')